...
import pandas as pd
import asyncio
from src.otodom_scraper import main_otodom
...
def get_path_otodom(folder_path, number, formatted_date):
    return f'{folder_path}//otodom_sale_from_{number}_days_{formatted_date}.xlsx'

def scrap_data_otodom(offer_type,selected_option_city, number, option_type, option_market_type, area):
    today_date = datetime.today()
    formatted_date = today_date.strftime("%Y-%m-%d-%H-%M")
    PATH_SALE = get_path_otodom(FOLDER_PATH_OTODOM, number, formatted_date)
    result_otodom = asyncio.run(main_otodom(offer_type,selected_option_city, number, option_type, option_market_type, area))
